...
- [`gget reactome`](reactome.md): **New module** to query the [Reactome](https://reactome.org/) pathway knowledgebase via its ContentService REST API. Supports three query types via the `resource` argument: `pathways` (the Reactome pathways an identifier such as a UniProt accession participates in), `search` (full-text search of the knowledgebase), and `entity` (details for a Reactome stable ID). Resolves [issue 114](https://github.com/scverse/gget/issues/114).
